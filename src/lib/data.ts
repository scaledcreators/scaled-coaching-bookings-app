...
const memberProfileCache = new Map<string, { expiresAt: number; profile: MemberProfile | null }>();
...
async function getMemberProfile(userId: string, companyId: string) {
  ...
  try {
    ...
    memberProfileCache.set(key, { expiresAt: Date.now() + 10 * 60_000, profile });
    return profile;
  } catch {
    memberProfileCache.set(key, { expiresAt: Date.now() + 60_000, profile: null });
    return null;
  }
}
...
export async function getCompanyData(companyId: string): Promise<DashboardData> {
  ...
  const memberProfiles = await getMemberProfiles((bookings.data ?? []).map((booking) => booking.whop_user_id), companyId);
  ...
  return {
    ...
    bookings: (bookings.data ?? []).map((booking) => ({ ...booking, member_profile: memberProfiles.get(booking.whop_user_id) ?? null })),
    unavailable: unavailable.data ?? [],
    coaches: coaches.data ?? [],
    availability: availability.data ?? [],
    settings: settings.data ? { ...settings.data, support_contact: settings.data.support_contact || DEFAULT_SUPPORT_CONTACT } : { emergency_paused: false, default_timezone: "America/Chicago", support_contact: DEFAULT_SUPPORT_CONTACT },
