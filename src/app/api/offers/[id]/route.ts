...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: offer, error } = await supabase.from("booking_offers").update({ title: input.title, description: input.description || null, duration_minutes: input.durationMinutes, price_cents: input.pricing === "paid" ? input.priceCents : 0, access_mode: input.pricing, status: input.status, min_notice_hours: input.minNoticeHours, max_advance_days: input.maxAdvanceDays, buffer_before_minutes: input.bufferBeforeMinutes, buffer_after_minutes: input.bufferAfterMinutes, delivery_mode: input.deliveryMode, intake_schema: input.intakeSchema, updated_at: new Date().toISOString() }).eq("id", id).eq("whop_company_id", input.companyId).select("*").single();
