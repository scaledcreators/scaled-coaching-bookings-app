...
const schema = z.object({ companyId: z.string().startsWith("biz_"), status: z.enum(statuses), meetingLocation: z.string().max(500).optional(), meetingUrl: z.url().optional(), joinInstructions: z.string().max(2000).optional() });
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (input.status === "confirmed") {
      ...
      const { data: blocked, error } = await supabase.rpc("is_booking_slot_blocked", { p_company_id: input.companyId, p_offer_id: existing.data.offer_id, p_coach_id: existing.data.coach_id, p_starts_at: start, p_ends_at: end, p_ignore_booking_id: id });
    ...
    if (error) throw error;
    await supabase.from("booking_messages").insert({ booking_request_id: id, sender: "system", body: `Booking status changed to ${input.status}.` });
