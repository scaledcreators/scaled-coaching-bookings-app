...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: booking, error } = await supabase.from("booking_requests").select("*").eq("id", id).eq("whop_company_id", companyId).eq("whop_user_id", viewer.userId).single();
    ...
    const { data, error: updateError } = await supabase.from("booking_requests").update({ status: "cancelled", refund_status: "requested", refund_reason: input.reason, refund_requested_at: now, updated_at: now }).eq("id", id).select("*, booking_offers(title,duration_minutes)").single();
