...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error: updateError } = await supabase.from("booking_requests").update({ status: "cancelled", refund_status: "processing", updated_at: now }).eq("id", id).select("*, booking_offers(title,duration_minutes,price_cents,access_mode,delivery_mode)").single();
