...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: booking, error } = await supabase.from("booking_requests").select("*, booking_offers(title,duration_minutes,min_notice_hours,max_advance_days)").eq("id", id).eq("whop_company_id", companyId).eq("whop_user_id", viewer.userId).single();
    ...
    const end = new Date(start.getTime() + offer.duration_minutes * 60_000); const blocked = await supabase.rpc("is_booking_slot_blocked", { p_company_id: companyId, p_offer_id: booking.offer_id, p_coach_id: booking.coach_id, p_starts_at: start.toISOString(), p_ends_at: end.toISOString(), p_ignore_booking_id: id }); if (blocked.error) throw blocked.error; if (blocked.data) return Response.json({ error: "That time is unavailable." }, { status: 409 });
